...
function isLoginValid (login, min = 4, max = 16) {
    ...
    const loginValid = (login.length >= min && login.length <= max) ? true : false;
    return loginValid;
  }
...
function isLoginUnique  (allLogins, login) {
  'use strict';
    ...
    if (LoginUnique === true) { return false };
    return true;
  }
...
function addLogin (allLogins, login) {
  ...
  let message;
    ...
    if (isLoginUnique(allLogins, login) === true) { allLogins.push(login); return message = SUCCESS }
    else if (isLoginValid(login) === false) { return (message = ERROR) }
    else {message = REFUSAL}
...
function isLoginValid (login, min = 4, max = 16) {
    if (login.length >= min && login.length <= max) { return true }
    else { return false}
}
//   console.log(isLoginValid('Kostya'))

function isLoginUnique(allLogins, login) {
    for (let i = 0; i <= allLogins.length; i += 1) { 
        if (allLogins[i].includes(login)) { return false  }
        else { return true}
    }
}
//   console.log(isLoginUnique(['Mango', 'robotGoogles', 'Poly', 'Aj4x1sBozz', 'qwerty123'], 'Kostya'))
...
function addLogin (allLogins, login) {
  ...
  const ERROR = 'Ошибка! Логин должен быть размером от 4 до 16 символов';
    ...
    if (!isLoginValid(login)) { return message = ERROR }
    else if (!isLoginUnique(allLogins, login)) { return message = REFUSAL }
    else { allLogins.push(login); return message = SUCCESS  }
